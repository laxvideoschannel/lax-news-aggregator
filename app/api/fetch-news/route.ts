...
export async function GET() {
  ...
  // 1. Safety Check: If we are in the "Build" phase, just stop and say okay.
  if (!url || !key || url === "undefined") {
    return new Response("Syncing with database...", { status: 200 });
  }

  try {
    // 2. Only create the client INSIDE the function
    ...
    for (const item of data.items) {
      ...
      if (content.includes('pll') || content.includes('wll') || content.includes('pro') || content.includes('chaos')) category = 'Pro';
      else if (content.includes('ncaa') || content.includes('d1') || content.includes('college') || content.includes('tusculum')) category = 'College';
